...
def make_observation(prompt) -> dict | None:
...
class GalbotController:
    def __init__(self, logger):
        ...
        robot_cfg = CFG['robot']
        socket_callback, sock = create_socket_callback(robot_cfg['ip'], robot_cfg['port'], timeout=20.0)
    ...
    def execute_action_realtime_clean(self, actions: np.ndarray):
        gripper_cfg = CFG['robot']['gripper']
        offset = gripper_cfg['offset']
        max_limit = gripper_cfg['action_limit']
    ...
    def left_gripper_close(self):
        self.interface.set_gripper_status(width_percent=0.005, speed=0.2, force=20, gripper="left_gripper")
    
    def right_gripper_close(self):
        self.interface.set_gripper_status(width_percent=0.005, speed=0.2, force=20, gripper="right_gripper")
...
def run_inference_loop():
    ...
    # start place_left task
    rospy.loginfo("[Robot] --------------Starting PLACE_LEFT_TASK--------------")
    rospy.loginfo("[Robot] Reset to initial place pose")
    controller.interface.set_leg_joint_angles(
        leg_joint_angles=CFG['place_spider_on_workshop']['init_leg_joints'], 
        speed=0.3, 
        asynchronous=True
    )
    time.sleep(5)
    controller.interface.set_arm_joint_angles(
        arm_joint_angles=CFG['place_spider_on_workshop']['init_place_left_arm_joints'], 
        speed=0.3, 
        arm="left_arm", 
        asynchronous=True
    )
    time.sleep(2)
    controller.interface.set_arm_joint_angles(
        arm_joint_angles=CFG['place_spider_on_workshop']['init_place_right_arm_joints'], 
        speed=0.3, 
        arm="right_arm", 
        asynchronous=True
    )
    time.sleep(2)
    controller.right_gripper_close()
    controller.left_gripper_close()
    time.sleep(2)

    # 使用 input() 阻塞程序等待输入
    print("\n" + "="*60)
    print("[Robot] Ready to proceed to: [Pick Spider to left workshop]?")
    user_input = input(">>> type 'y' to confirm, any other key to abort: ").strip().lower()
    print("="*60 + "\n")
    
    if user_input != 'y':
        rospy.logwarn("[Robot] User aborted. Stopping workflow.")
        return

    try:
        policy_client = WebSocketPolicyClient(CFG['inference']['ws_url'])

        while step < max_steps and not rospy.is_shutdown():
            time.sleep(0.1)
            obs = make_observation(CFG['inference']['task1_prompt'])
            if obs is None:
                time.sleep(0.05)
                continue
            ...
            controller.execute_action_realtime(actions[:30], is_placed_state=task_sm.is_placed())

            time.sleep(0.9)
            rospy.loginfo(f"---- executing step {step} | State: {task_sm.get_state_name()} ----")
            step += 1
    except KeyboardInterrupt:
        rospy.loginfo("[Robot] --------------Catched Ctrl_C, try to end PLACE_LEFT_TASK--------------")
    
    rospy.loginfo("[Robot] --------------need user to check whether go to next step--------------")
    # 使用 input() 阻塞程序等待输入
    print("\n" + "="*60)
    print("[Robot] Ready to proceed to: [Pick Spider from workshop]?")
    user_input = input(">>> type 'y' to confirm, any other key to abort: ").strip().lower()
    print("="*60 + "\n")
    ...
    rospy.loginfo("[Robot] --------------Starting [Pick Spider from workshop] TASK--------------")

    time.sleep(2)
    try:
        controller.interface.set_arm_joint_angles(arm_joint_angles=CFG['pick_spider_from_workshop']['pick_left_arm_joints_wp1'], speed=0.3, arm="left_arm", asynchronous=False)
    except Exception as e:
        rospy.logerr(f"[Robot] Error moving to pick_wp1: {e}")
    time.sleep(2)
    try:
        controller.interface.set_arm_joint_angles(arm_joint_angles=CFG['pick_spider_from_workshop']['pick_left_arm_joints_wp2'], speed=0.3, arm="left_arm", asynchronous=False)
    except Exception as e:
        rospy.logerr(f"[Robot] Error moving to pick_wp2: {e}")
    time.sleep(2)
    try:
        controller.left_gripper_close()
    except Exception as e:
        rospy.logerr(f"[Robot] Error closing left gripper: {e}")
    time.sleep(2)
    try:
        controller.interface.set_arm_joint_angles(arm_joint_angles=CFG['pick_spider_from_workshop']['lift_spider_joints'], speed=0.3, arm="left_arm", asynchronous=False)
    except Exception as e:
        rospy.logerr(f"[Robot] Error moving to lift_spider_joints: {e}")

    rospy.loginfo("[Robot] --------------End [Pick Spider from workshop] TASK--------------")

    rospy.loginfo("[Robot] --------------need user to check whether go to next step--------------")
    # 使用 input() 阻塞程序等待输入
    print("\n" + "="*60)
    print("[Robot] Ready to proceed to: [Place Spder to Tray]?")
    user_input = input(">>> type 'y' to confirm, any other key to abort: ").strip().lower()
    print("="*60 + "\n")
    ...
    if user_input != 'y':
        rospy.logwarn("[Robot] User aborted. Stopping workflow.")
        return
    rospy.loginfo("[Robot] --------------Starting PLACE_SPIDER_TO_TRAY TASK--------------")
    rospy.loginfo("[Robot] Reset to initial place pose")
    try:
        controller.interface.set_leg_joint_angles(leg_joint_angles=CFG['place_spider_on_tray']['init_leg_joints'], speed=0.3, asynchronous=True)
    except Exception as e:
        rospy.logerr(f"[Robot] Error moving to leg_joints: {e}")

    try:
        controller.interface.set_arm_joint_angles(arm_joint_angles=CFG['place_spider_on_tray']['init_place_arm_joints'], speed=0.3, arm="left_arm", asynchronous=True)
    except Exception as e:
        rospy.logerr(f"[Robot] Error moving to place_arm_joints: {e}")
    time.sleep(2)
    rospy.loginfo("[Robot] rotate leg to face tray")

    try:
        controller.interface.set_arm_joint_angles(arm_joint_angles=CFG['place_spider_on_tray']['init_place_arm_joints1'], speed=0.3, arm="left_arm", asynchronous=False)
    except Exception as e:
        rospy.logerr(f"[Robot] Error moving to pre_place_arm_joints: {e}")
    time.sleep(2)
    
    try:
        controller.interface.set_leg_joint_angles(leg_joint_angles=CFG['place_spider_on_tray']['right_arm_obs'], speed=0.3, arm="right_arm", asynchronous=False)
    except Exception as e:
        rospy.logerr(f"[Robot] Error moving to right_arm_obs: {e}")
    # time.sleep(2)
    # rospy.loginfo("[Robot] Starting PLACE_SPIDER_TO_TRAY inference loop")
    # step = 1

    # print("\n" + "="*60)
    # print("[Robot] Ready to proceed to: [Place Spder to Tray, Model Inference]?")
    # user_input = input(">>> type 'y' to confirm, any other key to abort: ").strip().lower()
    # print("="*60 + "\n")
    ...
    #     rospy.logwarn("[Robot] User aborted. Stopping workflow.")
    #     return
    # rospy.loginfo("[Robot] --------------Starting PLACE_SPIDER_TO_TRAY MODEL INFERENCE TASK--------------")
    
    # try:
    #     policy_client = WebSocketPolicyClient(CFG['inference']['ws_url_tray'])

    #     while step < max_steps:
    #         time.sleep(0.1)
    #         obs = make_observation(CFG['inference']['task2_prompt'])
    #         if obs is None:
    #             time.sleep(0.05)
    #             continue
    ...
    #         controller.execute_action_realtime_clean(actions[:action_horizon])

    #         time.sleep(1.0)
    #         rospy.loginfo(f"---- executing step {step} | State: {task_sm.get_state_name()} ----")
    #         step += 1
    # except KeyboardInterrupt:
    #     rospy.loginfo("[Robot] --------------Catched Ctrl_C, try to end PLACE_SPIDER_TO_TRAY--------------")
    
    rospy.loginfo("[Robot] --------------End PLACE_SPIDER_TO_TRAY TASK--------------")
