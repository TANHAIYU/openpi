...
import openpi.policies.galbot_g1_policy as galbot_policy
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "image": "observation.images.head_camera",
                        "wrist_image": "observation.images.left_camera",
                        "state": "observation.state",
                        "actions": "action",
                        "prompt": "prompt",
                    }
        ...
        data_transforms = _transforms.Group(
            ...
            outputs=[robotwin_policy.RobTwinOutputs()],
        )
...
class LeRobotGalbotDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # you can uncomment the following line to convert the actions to delta actions. The only exception
        # is for the gripper actions which are always absolute.
        # In the example below, we would apply the delta conversion to the first 7 actions (joints) and
        # leave the 8th action (gripper) unchanged, i.e. absolute.
        # In Libero, the raw actions in the dataset are already delta actions, so we *do not* need to
        # apply a separate delta conversion (that's why it's commented out). Choose whether to apply this
        # transform based on whether your dataset uses ``absolute`` or ``delta`` actions out of the box.
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        # weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
        weight_loader=weight_loaders.CheckpointWeightLoader("/home/data_sdd/weights/pi0_ckpt/galbot_ld/pi0_galbot_low_mem_finetune/galbot_ld_1214_foundation_mode_3c_5090/110000"),
    ...
    TrainConfig(
        name="pi0_robotwin_low_mem_finetune",
        model=pi0_config.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotRoboTwinDataConfig(
            repo_id="adjust_bottle/adjust_bottle_clean",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=True,
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
